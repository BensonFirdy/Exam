...
class User:
    ...
    def validate_register(form_data):
        ...
        if len(form_data["first_name"]) < 1:
            ...
            flash("First name must be between 2-25 characters long!")
            is_valid = False
        if len(form_data["last_name"]) < 1:
            flash("Last name must be present!")
            is_valid = False
        elif len(form_data["last_name"]) < 2 or len(form_data["last_name"]) > 25:
            flash("Last name must be between 2-25 characters long!")
            is_valid = False
